...
import AceProCard from '@/components/widgets/acepro/AceProCard.vue'
...
export default class Dashboard extends Mixins(StateMixin) {
  ...
  get supportsAcePro (): boolean {
    return this.$typedGetters['printer/getSupportsAcePro']
  }
  ...
  filtered (item: LayoutConfig) {
    ...
    if (item.id === 'ace-pro-card' && !this.supportsAcePro) return true

 ...
 */
  ...
  getRunoutSensors: (state): RunoutSensor[] => {
    return Object.keys(state.printer)
      .filter(key => (
        key.startsWith('filament_switch_sensor ') ||
  ...
  */
  ...
  getSupportsAcePro: (state, _getters, rootState): boolean => {
    ...
    const hasAceObject = Object.keys(printer).some((key) => {
      ...
      return value != null && (Array.isArray(value.slots) || value.dryer != null)
    })

    const hasAceConfig = Object.keys(printer.configfile?.settings ?? {})
      .some(key => /^ace(?:\s|$)/i.test(key))
